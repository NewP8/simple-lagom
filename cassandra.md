...
Struttura delle tabelle create da Lagom

## messages

```

    CREATE TABLE simple.messages (
       persistence_id text,
        partition_nr bigint,
        sequence_nr bigint,
        timestamp timeuuid,
...
```
CREATE TABLE simple.tag_scanning (
       persistence_id text,
        sequence_nr bigint,
        PRIMARY KEY (persistence_id)
    ) WITH read_repair_chance = 0.0 
...
```
    CREATE TABLE simple.tag_write_progress (
       persistence_id text,
        tag text,
        offset timeuuid,
        sequence_nr bigint,
