...
/**
  ...
  */
object SimpleBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[SimpleCommand]): Behavior[SimpleCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, SimpleEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[SimpleCommand, SimpleEvent, SimpleState](
        persistenceId = persistenceId,
        emptyState = SimpleState.initial,
...
/**
  ...
  */
case class SimpleState(message: String, timestamp: String) {
  def applyCommand(cmd: SimpleCommand): ReplyEffect[SimpleEvent, SimpleState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: SimpleEvent): SimpleState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[SimpleEvent, SimpleState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[SimpleEvent, SimpleState] =
    Effect
...
object SimpleState {
  ...
  /**
    ...
    */
  def initial: SimpleState = SimpleState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[SimpleCommand]("SimpleAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[SimpleState] = Json.format
}

/**
  * This interface defines all the events that the SimpleAggregate supports.
  */
sealed trait SimpleEvent extends AggregateEvent[SimpleEvent] {
  def aggregateTag: AggregateEventTag[SimpleEvent] = SimpleEvent.Tag
}

object SimpleEvent {
  val Tag: AggregateEventTag[SimpleEvent] = AggregateEventTag[SimpleEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends SimpleEvent
...
/**
  ...
  */
trait SimpleCommandSerializable

/**
  * This interface defines all the commands that the SimpleAggregate supports.
  */
sealed trait SimpleCommand
    extends SimpleCommandSerializable
